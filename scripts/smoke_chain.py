...
def main() -> int:
    ...
    dep = next((d for d in watchlist.dependencies if d.name == demo["dependency"]), None)
    if dep is None:
    ...
    print(f"             {demo['from_version']} -> {demo['to_version']} is newer: {newer}")
    ...
    red = parse_pytest((ROOT / "fixtures/pytest_output_red.txt").read_text(),
                       package=dep.name, version=demo["to_version"], report_id="red")
    green = parse_pytest((ROOT / "fixtures/pytest_output_green.txt").read_text(),
                         package=dep.name, version=demo["from_version"], report_id="green")
    print(f"3. REPRODUCE red   broken={red.is_broken} green={red.is_green} "
          f"(passed={red.passed} failed={red.failed} errors={red.errors})")
    print(f"             green broken={green.is_broken} green={green.is_green} "
          f"(passed={green.passed})")
    if not (red.is_broken and not red.is_green and green.is_green):
        print("             the red/green pair is not behaving — the gate would misfire")
        return 1

    # STORE — contracts survive a round trip.
    store = SqliteStore(":memory:")
    mission = store.create_mission(ReleaseEvent(
        dependency=dep.name, version=demo["to_version"],
        published_at="2026-08-01T00:00:00Z", title=f"{dep.name} {demo['to_version']}",
        body="breaking change", url=demo["repo_url"], breaking_hint=True,
        source_collector=None))
    store.save_report(mission.id, red)
    store.save_report(mission.id, green)
    for point in points[:3]:
        store.save_impact(mission.id, ImpactRow(
            contact_point=point, verdict=Verdict.BROKEN,
            why="module fails to import under the new version", evidence_ref=red.id))
    loaded = store.get_mission(mission.id)
    print(f"4. STORE     {len(loaded.impact_rows)} impact rows, "
